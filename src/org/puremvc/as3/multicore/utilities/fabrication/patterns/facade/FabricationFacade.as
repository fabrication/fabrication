...
package org.puremvc.as3.multicore.utilities.fabrication.patterns.facade {
    ...
    public class FabricationFacade extends Facade implements IDisposable {
        ...
        override public function registerMediator(mediator:IMediator):void
        {
            ...
            var fabricationMediator:FabricationMediator = mediator as FabricationMediator;
            if (fabricationMediator) {

                var mediatorClassName:String = getQualifiedClassName( fabricationMediator );
                if ( !fabricationMediator.passedNotificationRegistrationPhase ) {

                    logger.warn("[FabricationMediator.listNotifications] is overwritten, fabriction reflexive notifications are disabled in [ " + mediatorClassName + "]");
                }
                if ( !fabricationMediator.passedReactionsRegistrationPhase ) {

                    logger.warn("[FabricationMediator.onRegister] should be overwritten with super.onRegister() call to support reactions in [ " + mediatorClassName + "]");
                }
            }
        }
